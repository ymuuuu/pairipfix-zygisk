...
// helper.dex must be generated by helper/build-dex.sh before building. An empty
// kHelperDex silently disables every Java hook (InMemoryDexClassLoader finds no
// classes), so fail the build loudly instead of shipping an inert module.
static_assert(kHelperDexLen > 0,
    "helper.dex is empty: run helper/build-dex.sh to generate native/jni/helper_dex.h");
...
// Retained global ref: pins the anchor hooker object so LSPlant's hook on
// Instrumentation.callApplicationOnCreate stays valid for the process lifetime.
// The anchor is intentionally never unhooked (see Anchor_onCreate note in main.cpp).
...
void InstallJavaHooks(JNIEnv* env, jobject appCl, const char* selfPackage) {
    // LSPlant is already initialized in postAppSpecialize; InitLSPlant is idempotent.
